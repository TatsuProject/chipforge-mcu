...
module alu (
  ...
  assign clmul_e  = (alu_ctrl == CLMUL ) | 
                    (alu_ctrl == CLMULH) ;

  assign xperm4_e = (alu_ctrl == XPERM4) ;
  assign xperm8_e = (alu_ctrl == XPERM8) ;
  assign brev8_e  = (alu_ctrl == BREV  ) ;
  assign rev8_e   = (alu_ctrl == REV   ) ;
  assign zip_e    = (alu_ctrl == ZIP   ) ;
  assign unzip_e  = (alu_ctrl == UNZIP ) ;
